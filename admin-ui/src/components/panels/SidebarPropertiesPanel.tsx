...
import { type TabColorTheme, type AppSidebarTabValue } from '../layout/tab-colors';
import styles from './sidebar-properties-panel.module.css';
...
export function SidebarPropertiesPanel({ activeColor, activeTab = 'themes' }: SidebarPropertiesPanelProps): JSX.Element {
  ...
  // Gate inspectors by activeTab - Sidebar-specific tabs only
  const isSidebarIntegrationTab = activeTab === 'integration';
  ...
  if (selectedWidgetId) {
    ...
    if (selectedWidgetId === 'page:footer') {
      ...
      if (isFeaturedWidget) {
        ...
        // Standard widgets are now edited in the Modal (ContentEditorModal), so we don't show the inspector in the sidebar
        inspector = null;
      }
    } else {
  ...
  } else if (isSidebarIntegrationTab) {
