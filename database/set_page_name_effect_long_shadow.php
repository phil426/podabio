<?php
/**
 * Script to manually set page_name_effect to 'long-shadow' for testing
...
echo "<!DOCTYPE html><html><head><title>Set Page Name Effect to Long Shadow</title>";
...
echo "<h1>Set Page Name Effect to 'long-shadow'</h1>";
...
try {
    ...
    $update->execute(['long-shadow', $pageId]);
    echo "<div class='success'>✓ Updated page_name_effect to 'long-shadow' for page ID {$pageId}</div>";
    ...
    echo "<div class='info'>→ Now check editor.php?tab=appearance - the 'Long Shadow' option should be selected</div>";
    echo "<div class='info'>→ Also check your public page - it should display the long shadow effect</div>";
} catch (PDOException $e) {
